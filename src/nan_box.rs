...
impl NanBox64 {
    ...
    #[inline]
    pub const fn new_raw_tag(raw_tag: u64) -> Self {
        Self {
            tagged: raw_tag | (u64::MAX & EXP_FIELD_MASK) | raw_tag,
        }
    }
    ...
    #[inline]
    pub const unsafe fn new_tag(tag: TagBuilder) -> Self {
        Self {
            tagged: tag.0,
        }
    }
    ...
    pub const fn is_tagged(self) -> bool {
        self.tagged & !(SIGN_MASK | ARBITRARY_FIELD_MASK) > EXP_FIELD_MASK
    }
...
impl Tag {
    ...
    pub const fn non_zero_field(self) -> NonZeroU64 {
        unsafe { NonZeroU64::new_unchecked((self.0 & NON_ZERO_FIELD_MASK) >> NON_ZERO_FIELD_MASK.leading_zeros()) }
    }
...
/// This field is allowed to store 50 non-zero bits of information in its 50 LSB
...
impl TagBuilder {
    ...
    pub const fn sign(mut self, sign: bool) -> Self {
        let sign = unsafe { transmute::<bool, u8>(sign) } as u64;
        self.0 |= sign;
